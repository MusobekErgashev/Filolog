      ...
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <div className="flex flex-row items-end justify-center gap-2 sm:gap-4 lg:gap-8 pt-10">
          {podiumOrder.map((user, index) => {
            ...
            return (
              ...
              >
                {/* Badge/Icon */}
                <div className={`absolute -top-4 sm:-top-6 p-2 sm:p-4 rounded-xl sm:rounded-2xl shadow-lg transform transition-transform group-hover:rotate-12 ${isFirst ? 'bg-indigo-600 text-white' :
                ...
                {/* Trend Tag */}
                {user.trend && (
                  <div className={`hidden sm:flex mt-4 px-4 py-1 rounded-full text-xs font-black items-center gap-1 ${user.trend === 'up' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'
                    }`}>
        ...
        <div className="bg-white rounded-[24px] sm:rounded-[40px] shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden mt-8 sm:mt-12 mb-20 animate-in fade-in duration-700">
          <div className="px-5 sm:px-8 py-4 sm:py-6 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
            <h2 className="text-base sm:text-xl font-black text-slate-800">Top 10 ta ishtirokchilar</h2>
